...
puum_area <- fs::dir_ls(local_path, recurse = TRUE, glob = '*NHDArea.shp')
puum_fl <- fs::dir_ls(local_path, recurse = TRUE, glob = '*NHDFlowline.shp')
puum_wb <- fs::dir_ls(local_path, recurse = TRUE, glob = '*NHDWaterbody.shp')
puum_huc2 <- fs::dir_ls(local_path, recurse = TRUE, glob = '*HU2.shp')
puum_huc8 <- fs::dir_ls(local_path, recurse = TRUE, glob = '*HU8.shp')

puum_area_sf <- st_read(puum_area) %>% st_crop(st_zm(huc8_sf))
puum_fl_sf <- st_read(puum_fl) %>% st_zm() %>% st_crop(st_zm(huc8_sf))
puum_wb_sf <- st_read(puum_wb) %>% st_zm() %>% st_crop(st_zm(huc8_sf))
puum_huc2_sf <- st_read(puum_huc2)
my_huc8 <- st_read(puum_huc8) %>% st_zm() %>% st_crop(st_zm(huc8_sf))
...
ggsave(glue('figs/{my_site}-{huc2_id}.png'))


ggplot() + 
  geom_sf(data = huc8_sf, aes(fill = Name)) +
  geom_sf(data = puum_wb_sf, fill = 'dodgerblue', col = 'blue', lwd = 0.1) +
...
ggsave(glue('figs/nhd-waterbodies-{my_site}-{huc2_id}.png'))

ggplot() + 
  geom_sf(data = huc8_sf, aes(fill = Name)) +
  # geom_sf(data = my_wb_sf, fill = 'dodgerblue', col = 'dodgerblue') +
  geom_sf(data = puum_fl_sf, col = 'blue') +
  geom_sf(data = my_site_sf, col = 'black', fill = NA) +
  ggtitle(glue('{my_site} flightbox NHD HUC8 flowlines')) +
  theme_void()

ggsave(glue('figs/nhd-lines-{my_site}-{huc2_id}.png'))

# ggplot() + 
#   geom_sf(data = huc8_sf, aes(fill = Name)) +
#   geom_sf(data = puum_area_sf, col = 'blue') +
#   geom_sf(data = my_site_sf, col = 'black', fill = NA) +
#   ggtitle(glue('{my_site} flightbox NHD HUC8 area')) +
#   theme_void()
# 
# ggsave(glue('figs/nhd-area-{my_site}-{huc2_id}.png'))

# Calculate areas 

fl_in_fbox <- st_intersection(my_site_sf, st_zm(puum_fl_sf))
wb_in_fbox <- st_intersection(my_site_sf, st_zm(puum_wb_sf))
...
ggsave(glue('figs/{my_site}-NHD-{huc2_id}.png'))
...
neon_sites_in_huc2[5]
huc2_paths <- names(neon_sites_in_huc2)

nhd_stuff_for_site(neon_sites_in_huc2[[1]][1], huc2_path[1])

nhd_stuff_for_site('UNDE', huc2_paths[7])
nhd_stuff_for_site('UNDE', huc2_paths[2])
nhd_stuff_for_site('NIWO', huc2_paths[10])
nhd_stuff_for_site('NIWO', huc2_paths[14])
nhd_stuff_for_site('MLBS', huc2_paths[2])
nhd_stuff_for_site('MLBS', huc2_paths[5])
...
For any site that spans multiple HUC2s this won't 

re-do for: CRAM/UNDE, NIWO/COMO